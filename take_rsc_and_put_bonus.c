...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/04 20:25:58 by osouadi           #+#    #+#             */
/*   Updated: 2025/01/04 20:26:01 by osouadi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	player_img(t_data *data)
{
	...
	if (!(data->character_img))
		return (0);
	data->character1 = mlx_xpm_file_to_image(data->mlx_ptr,
			"./textures/Enemy_1.xpm", &width, &height);
	if (!(data->character1))
		return (ft_free_img(data, data->character_img), 0);
	data->character2 = mlx_xpm_file_to_image(data->mlx_ptr,
			"./textures/Enemy_2.xpm", &width, &height);
	if (!(data->character2))
	{
		ft_free_img(data, data->character_img);
		return (ft_free_img(data, data->character1), 0);
	}
	return (0);
}
...
static int	map_img(t_data *data)
{
	...
	if (!(data->collection_img))
	{
		return (ft_free_img3(data, data->floor_img, data->wall_img,
				data->collection_img), 0);
	}
	data->enmy_img = ft_src_img(data, "./textures/enmy.xpm");
	if (!(data->collection_img))
	{
		ft_free_img3(data, data->floor_img, data->wall_img,
			data->collection_img);
		return (free(data->exit_img), 0);
	}
	return (0);
}
